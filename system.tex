...
For the subsequent frames, using the orientation change, we predict the region where the the traffic lights are likely to move using and process only that region for detecting the traffic lights.
...
To filter out such false detections, we used two heuristic filters, which utilize the fact that traffic lights are usually placed within a black box. 
